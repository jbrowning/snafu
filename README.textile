...
<pre lang="ruby">
    current_time = Snafu::Models::GlitchTime.new
    current_time.year
    => 20
    current_time.month
    => "Spork"
...
h3. Locations (no auth required)

Currently, only the locations.* methods are supported. Each method returns a Ruby object representing the returned data.
